			...
			<View style={modal.overlay}>
				<View style={modal.sheet}>
					...
					>
								...
								<View style={{ gap: 5, flex: 1 }}>
									<View style={[S.row, { gap: 6, flexWrap: "wrap" }]}>
										...
										<View style={[modal.diffPill, { borderColor: diffColor }]}>
											<Text style={[modal.diffText, { color: diffColor }]}>
						...
						<Text
							style={[S.heading, { fontSize: Fonts.sizes.xxl, lineHeight: 30 }]}
						>
							{mission.title}
						</Text>
						...
						>
							<Text
								style={[S.muted, { fontSize: Fonts.sizes.xs, marginBottom: 6 }]}
							>
						...
						{/* Description */}
						<Text
							style={[S.body, { fontSize: Fonts.sizes.md, lineHeight: 23 }]}
						>
							{mission.description}
						</Text>
					...
					<View style={{ paddingTop: 16, gap: 10 }}>
						...
						{mission.completed && !mission.claimed && (
							<TouchableOpacity
								style={[S.primaryBtn, { backgroundColor: Colors.xpGold }]}
								onPress={onClaim}
								activeOpacity={0.85}
							>
								<Text style={[S.primaryBtnText, { color: "#000" }]}>
...
function MissionTile({
	...
	if (mission.claimed)
		statusEl = (
			<Text style={[tile.status, { color: Colors.textMuted }]}>✅ done</Text>
		);
	else if (mission.completed)
		statusEl = (
			<Text style={[tile.status, { color: Colors.xpGold }]}>🎁 claim!</Text>
		);
	else if (loggedToday)
		statusEl = (
			<Text style={[tile.status, { color: Colors.primary }]}>
				✓ logged today
			</Text>
		);
	else if (mission.pledged)
		statusEl = (
			<Text style={[tile.status, { color: Colors.textMuted }]}>
				in progress
			</Text>
		);
	else
	...
	return (
		<TouchableOpacity
			style={[
				...
				mission.completed &&
					!mission.claimed && { borderColor: Colors.xpGold, borderWidth: 1.5 },
		...
		>
			...
			<Text
				style={[S.heading, { fontSize: Fonts.sizes.sm, lineHeight: 17 }]}
				numberOfLines={2}
			>
				{mission.title}
			</Text>
			<Text style={[S.muted, { fontSize: Fonts.sizes.xs, marginTop: 3 }]}>
				{daysLeft}d left
			</Text>
			...
			<View style={[tile.diffPill, { borderColor: diffColor, marginTop: 8 }]}>
				<Text style={[tile.diffText, { color: diffColor }]}>
					{mission.difficulty}
				</Text>
			</View>
