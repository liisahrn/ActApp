...
type LeaderboardUser = {
  ...
  xp: number; level: number; current_streak: number; country: string | null
}
...
export default function FriendsScreen() {
  ...
  const enrichWithStreaks = async (profiles: any[]): Promise<LeaderboardUser[]> => {
    ...
    data?.forEach(s => { map[s.user_id] = s.current_streak })
  ...
  const renderUser = ({ item, index }: { item: LeaderboardUser; index: number }) => {
    ...
    return (
      <View style={[S.rowCard, isMe && styles.meRow]}>
        ...
        <View style={S.fill}>
          <Text style={[S.heading, { fontSize: Fonts.sizes.md }]}>{item.username}{isMe ? ' · you' : ''}</Text>
          <Text style={[S.muted, { fontSize: Fonts.sizes.xs, marginTop: 2 }]}>
            Lv.{item.level} · 🔥{item.current_streak}{item.country ? ` · ${item.country}` : ''}
          </Text>
        </View>
        <View style={{ alignItems: 'flex-end', gap: 4 }}>
          <Text style={styles.xp}>⭐ {item.xp}</Text>
          {!isMe && (
            <TouchableOpacity style={[S.pillBtn, isFollowing && styles.followingBtn]} onPress={() => toggleFollow(item.id)}>
              <Text style={[S.pillBtnText, { fontSize: 10 }]}>{isFollowing ? 'Following' : 'Follow'}</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
  ...
  return (
    <View style={S.fill}>
      ...
      <View style={styles.header}>
        <Text style={[S.heading, { fontSize: Fonts.sizes.xxxl }]}>Agent Rankings</Text>
        <View style={styles.tabs}>
          {(['friends', 'global'] as Tab[]).map(tab => (
            <TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && styles.tabActive]}
              ...
              <Text style={[S.muted, { fontSize: Fonts.sizes.sm }, activeTab === tab && styles.tabTextActive]}>
      ...
      {activeTab === 'friends' && (
        <View style={{ paddingHorizontal: 20, paddingBottom: 8 }}>
          <TextInput style={S.input} placeholder="Find agents by username…"
            placeholderTextColor={Colors.textMuted} value={searchQuery} autoCapitalize="none"
      ...
      {searchResults.length > 0 && (
        <View style={[S.card, { marginHorizontal: 20, marginBottom: 8 }]}>
          {searchResults.map(item => (
            <View key={item.id} style={[S.row, { padding: 12, gap: 10, borderBottomWidth: 1, borderBottomColor: Colors.borderLight }]}>
              <Text style={{ fontSize: 24 }}>{item.avatar_url}</Text>
              <View style={S.fill}>
                <Text style={[S.heading, { fontSize: Fonts.sizes.md }]}>{item.username}</Text>
                <Text style={[S.muted, { fontSize: Fonts.sizes.xs }]}>Lv.{item.level} · ⭐{item.xp}</Text>
              </View>
              <TouchableOpacity style={[S.pillBtn, following.has(item.id) && styles.followingBtn]} onPress={() => toggleFollow(item.id)}>
                <Text style={[S.pillBtnText, { fontSize: 10 }]}>{following.has(item.id) ? 'Following' : 'Follow'}</Text>
              </TouchableOpacity>
            </View>
          ))}
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={S.empty}>
            <Text style={S.emptyEmoji}>{activeTab === 'friends' ? '👥' : '🌍'}</Text>
            <Text style={S.emptyText}>{activeTab === 'friends' ? 'No agents yet!' : 'No data yet'}</Text>
            <Text style={S.emptyHint}>{activeTab === 'friends' ? 'Search above to find and follow fellow agents.' : 'Be the first agent to deploy!'}</Text>
          </View>
        }
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  rank:         { fontFamily: Fonts.heading, color: Colors.textSecondary, fontSize: Fonts.sizes.lg, width: 32, textAlign: 'center' },
  xp:           { fontFamily: Fonts.heading, fontSize: Fonts.sizes.sm, color: Colors.xpGold },
