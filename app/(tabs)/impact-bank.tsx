...
export default function ImpactBankScreen() {
	...
	const { fetchMissions } = useActionStore();
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: fetchImpact is defined in component scope; fetchMissions is a stable store action; session is the correct trigger
	useEffect(() => {
		...
		<View className="flex-1">
			...
			>
					...
					<View style={{ paddingHorizontal: 20, paddingTop: 24, gap: 12 }}>
						...
						>
							{weeks.map((w) => (
								<View
									key={w.week}
									className="flex-1 items-center gap-1 justify-end"
									style={{ height: "100%" }}
								>
									<Text
										style={{
									...
									<View
										className="w-full overflow-hidden justify-end"
										style={{
						...
						<MutedText className="text-[10] text-right mt-1">
							kg CO₂ per week
						</MutedText>
					</View>
				)}
						...
						<View style={{ gap: 10 }}>
							{categories.map((c) => (
								<View key={c.category} className="flex-row items-center gap-2">
									...
									<Text
										className="font-body text-secondary text-sm capitalize"
										style={{ width: 80 }}
									>
										{c.category}
									</Text>
				...
				{!loading && totalActions === 0 && (
					<EmptyState
						emoji="🌱"
						title="No missions logged yet"
						hint="Complete your first daily mission to start building your field record."
					/>
				)}
