    ...
    <View style={S.fill}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <View style={S.missionBadge}><Text style={S.missionBadgeText}>IMPACT BANK</Text></View>
          <Text style={[S.heading, { fontSize: Fonts.sizes.xxxl }]}>Field Record</Text>
          <Text style={[S.muted, { fontSize: Fonts.sizes.sm }]}>Your mission history. Every deployment counted.</Text>
        </View>

        {/* ── Big CO₂ total ─────────────────────────────────────── */}
        <View style={styles.heroCard}>
          <Text style={[S.muted, { fontSize: Fonts.sizes.sm }]}>TOTAL CO₂ SAVED</Text>
          <Text style={styles.bigCO2}>{loading ? '…' : `${totalCO2}kg`}</Text>
          <Text style={[S.muted, { fontSize: Fonts.sizes.sm }]}>{totalActions} missions completed</Text>
        </View>
            ...
            <View style={styles.barChart}>
              {weeks.map((w, i) => (
                <View key={i} style={styles.barCol}>
                  <Text style={styles.barValue}>{w.co2 > 0 ? `${w.co2}` : ''}</Text>
                  <View style={styles.barTrack}>
                    <View style={[styles.barFill, { height: `${Math.max(4, (w.co2 / maxWeekCO2) * 100)}%` }]} />
                  </View>
                  <Text style={styles.barLabel}>{w.week}</Text>
                </View>
              ))}
            </View>
            <Text style={[S.muted, { fontSize: 10, textAlign: 'right', marginTop: 4 }]}>kg CO₂ per week</Text>
          </View>
        )}
            ...
            <View style={styles.catList}>
              {categories.map(c => (
                <View key={c.category} style={styles.catRow}>
                  ...
                  <Text style={[S.body, { width: 80, fontSize: Fonts.sizes.sm, textTransform: 'capitalize' }]}>
                    {c.category}
                  </Text>
                  <View style={styles.catBarTrack}>
                    <View style={[styles.catBarFill, { width: `${(c.count / maxCatCount) * 100}%` }]} />
                  </View>
                  <Text style={[S.muted, { fontSize: Fonts.sizes.xs, width: 28, textAlign: 'right' }]}>
                    {c.count}
                  </Text>
                </View>
              ))}
        ...
        {!loading && totalActions === 0 && (
          <View style={S.empty}>
            ...
            <Text style={S.emptyHint}>Complete your first daily mission to start building your field record.</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  // ── Hero CO₂ ───────────────────────────────────────────────────
  heroCard: { marginHorizontal: 20, backgroundColor: Colors.surfaceDark, borderRadius: Radius.xl, padding: 28, alignItems: 'center', gap: 6, borderWidth: 1, borderColor: Colors.primary + '40', ...Shadow.md },
  bigCO2:   { fontSize: 64, fontFamily: Fonts.heading, color: Colors.primary, letterSpacing: -2 },
  ...
  equivValue:{ fontSize: Fonts.sizes.xl, fontFamily: Fonts.heading, color: Colors.text },
  equivLabel:{ fontSize: 10, fontFamily: Fonts.body, color: Colors.textMuted, textAlign: 'center', lineHeight: 14 },
  ...
  barTrack:  { width: '100%', backgroundColor: Colors.surface, borderRadius: 4, overflow: 'hidden', height: 80, justifyContent: 'flex-end' },
  barFill:   { width: '100%', backgroundColor: Colors.primary, borderRadius: 4, opacity: 0.85 },
  barLabel:  { fontSize: 9, fontFamily: Fonts.body, color: Colors.textMuted, textAlign: 'center' },

  // ── Category ──────────────────────────────────────────────────
  catList:      { gap: 10 },
  catRow:       { flexDirection: 'row', alignItems: 'center', gap: 8 },
  catBarTrack:  { flex: 1, height: 8, backgroundColor: Colors.surface, borderRadius: 4, overflow: 'hidden' },
