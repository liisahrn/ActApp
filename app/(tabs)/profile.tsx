		...
		<View style={S.fill}>
			...
			<ScrollView showsVerticalScrollIndicator={false}>
				...
				<View style={styles.hero}>
					...
					<Text style={[S.heading, { fontSize: Fonts.sizes.xxl }]}>
						@{profile?.username}
					</Text>
					...
					<View style={{ width: "100%", gap: 6, marginTop: 8 }}>
						...
						<Text
							style={[
								S.muted,
								{ fontSize: Fonts.sizes.xs, textAlign: "right" },
							]}
						>
							{xp} / {nextXP} XP
						</Text>
				...
				<View style={styles.statsRow}>
					{STATS.map((s) => (
						<View key={s.label} style={[S.card, styles.statCard]}>
							...
							<Text style={[S.muted, { fontSize: 10, textAlign: "center" }]}>
								{s.label}
							</Text>
						</View>
					))}
				...
				<View style={{ padding: 20, gap: 14 }}>
					...
					{badges.length === 0 ? (
						<View style={[S.empty, { paddingTop: 32 }]}>
							...
							<Text style={[S.body, { fontSize: Fonts.sizes.md }]}>
								...
								{earnedBadges.map((b) => (
									<View key={b.id} style={[S.card, styles.badgeCard]}>
										...
										<Text
											style={[
												S.heading,
												{ fontSize: Fonts.sizes.xs, textAlign: "center" },
											]}
										>
											{b.name}
										</Text>
							...
							{lockedBadges.length > 0 && (
								<>
									<Text
										style={[
											S.muted,
											{ fontSize: Fonts.sizes.sm, marginTop: 4 },
										]}
									>
										Locked
									</Text>
									<View style={styles.badgeGrid}>
										{lockedBadges.map((b) => (
											<View
												key={b.id}
												style={[S.card, styles.badgeCard, { opacity: 0.45 }]}
											>
