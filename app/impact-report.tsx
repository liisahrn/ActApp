...
const toFlag = (c: string) => {
	...
	const o = 0x1f1e6 - 65;
	return (
		String.fromCodePoint(c.toUpperCase().charCodeAt(0) + o) +
...
const CO2_EQUIV = (kg: number) => [
	{
		emoji: "🌳",
		value: Math.max(1, Math.round(kg / 21)),
		unit: "trees",
		desc: "absorbing CO₂ for a year",
	},
	{
		emoji: "🚗",
		value: Math.max(1, Math.round(kg / 0.12)),
		unit: "km",
		desc: "not driven by car",
	},
	{
		emoji: "🏠",
		value: Math.max(1, Math.round(kg / 8.5)),
		unit: "homes",
		desc: "powered for a day",
	},
	{
		emoji: "📱",
		value: Math.max(1, Math.round(kg * 200)),
		unit: "phones",
		desc: "fully charged",
	},
];
...
export default function ImpactReportScreen() {
	...
	const equivAnims = useRef(
		[0, 1, 2, 3].map(() => new Animated.Value(0)),
	).current;
	const [countries, setCountries] = useState<CountryRow[]>([]);
	const [showEquiv, setShowEquiv] = useState(false);

	// biome-ignore lint/correctness/useExhaustiveDependencies: intentional run-once mount effect; equivAnims is a stable ref, fetchCountries takes its arg inline
	useEffect(() => {
	...
	const fetchCountries = async (actionId: string) => {
		...
		// biome-ignore lint/suspicious/noExplicitAny: supabase join return type varies without generated types
		data?.forEach((c: any) => {
			...
			>
				...
				>
					...
					{imageUrl ? (
						...
						<View
							className="items-center justify-center gap-2 border border-border bg-surface"
							style={{ height: 140, borderRadius: Radius.xl }}
						>
							<Text style={{ fontSize: 48 }}>🌍</Text>
							<MutedText className="text-sm">
								Impact image generating…
							</MutedText>
						</View>
					)}
					...
					>
						...
						<BodyText className="text-center text-md">
							agents deployed today
						</BodyText>
						{co2 > 0 && (
							<Text
								style={{
								...
								>
									...
									>
										<View
											style={{
												height: "100%",
												width: `${Math.max(4, (row.completions / maxCountry) * 100)}%`,
